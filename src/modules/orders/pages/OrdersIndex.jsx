...
const OrdersIndex = () => {
  ...
  const exportToExcel = useCallback(() => {
    ...
    const exportData = data?.data?.orders?.flatMap((order) => {
      if (!order?.lines) return []; // Prevents errors if order.lines is missing
  ...
  const getStatusDetails = (status) => {
    switch (status) {
      ...
      case "cancel_requested":
        return { label: "request cancel", color: "error", icon: <Cancel /> };
      case "order_canceled":
...
`;
        ...
        return (
          ...
          >
            ...
            >
              ...
              <IconButton
                component="a"
                href={`${REACT_APP_API_URL}/order/${orders.id}/pdf`}
              >
  ...
  }, [REACT_APP_API_URL, data?.data?.orders, flipIn, handleView]);
