...
import { useDeleteProductSizes } from "hooks/productSizes/useDeleteProductSizes";
...
const DeleteDialog = ({ id, page, count }) => {
  ...
  const deleteproductSizes = useDeleteProductSizes({ page, count });
  const handleClickOpen = (e) => setOpen(true)
  const handleClose = () => setOpen(false)
  const { refetch } = useProductSizes();
  const DeleteProductSizes = () => {
    setLoading(true);
    deleteproductSizes.mutate(id, {
      onSuccess: () => {
        setOpen(false);
        refetch()
      },
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogActions>
          ...
          {loading && <Loader />}
          <Button autoFocus sx={{}} variant="contained" onClick={DeleteProductSizes}>
            {t("Agree")}
          </Button>
        </DialogActions>
      </Dialog>
