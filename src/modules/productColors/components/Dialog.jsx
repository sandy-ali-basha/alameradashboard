...
import { useDeleteProductcolors } from "hooks/productcolors/useDeleteProductcolors";
...
const DeleteDialog = ({ id, page, count }) => {
  ...
  const deleteproductcolors = useDeleteProductcolors({ page, count });
  const handleClickOpen = (e) => setOpen(true)
  const handleClose = () => setOpen(false)
  const { refetch } = useProductcolors();
  const DeleteProductcolors = () => {
    setLoading(true);
    deleteproductcolors.mutate(id, {
      onSuccess: () => {
        setOpen(false);
        refetch()
      },
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogActions>
          ...
          {loading && <Loader />}
          <Button autoFocus sx={{}} variant="contained" onClick={DeleteProductcolors}>
            {t("Agree")}
          </Button>
        </DialogActions>
      </Dialog>
